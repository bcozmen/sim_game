...
from ..path.path_finding import dijkstra, astar
...
class City:
    def __init__(self, id=1, maps=None, max_radius=150, max_road_radius=10, growth_factor={}):
        self.id = id
        self.maps = maps
    ...
    def grow(self, amount, preference="fertility", land_type=0, plot=False):
        ...
        growth_score, growth_mask = self.calculate_score(cut_maps, land_type=land_type)
        chosen_cells, global_cells = self.choose_growth_cells(cut_maps, growth_score, growth_mask, amount)
        if chosen_cells is False:
            return False
        self.register(global_cells, land_type=land_type, preference=preference)
        if land_type == 1:
            self.construct_roads(cut_maps, chosen_cells[0])

    def calculate_score(self, cut_maps, land_type=0):
        distance_score = self.calculate_distance(cut_maps)
        preference_score = cut_maps["fertility"]
        growth_mask = self.get_growth_mask(cut_maps, land_type)
        growth_score = (
            preference_score ** self.growth_factor["preference"][land_type]
            * distance_score ** self.growth_factor["distance"][land_type]
            * growth_mask
        )
        return growth_score, growth_mask

    def choose_growth_cells(self, maps, growth_score, growth_mask, amount):
        chosen_cells = []
        while len(chosen_cells) < amount:
            new_cells, growth_mask = self.grow_into(maps, growth_score, amount, growth_mask)
            chosen_cells.extend(new_cells)
            if len(new_cells) == 0:
                break
    ...
    def grow_into(self, maps, growth_score, amount, growth_mask, land_type=0):
        ...
        coef = 10 if land_type == 1 else 50
        cost_map = 1 + coef * (maps["directional_slope"] ** 2)
        cost_map[~growth_mask] = np.inf

        _, dist, _ = dijkstra(cost_map, index, goals=None)
    ...
    def calculate_distance(self, cut_maps):
        ...
        _, dist, _ = dijkstra(cost_map, cut_maps["urban_mask"], goals=None)
    ...
    def init_location(self):
        ...
        city_mask = build_city_exclusion_mask(city_map, self.max_radius)
