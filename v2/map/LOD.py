...
class Terrain:
    ...
    def generate(self,
        ...
        print("mean delta:", np.mean(np.abs(terrain - before)))
        print("max delta:", np.max(np.abs(terrain - before)))
        return terrain
    ...
    def plot3D(self, terrain, zlim=(-1, 2)):
        ...
        # LAYERED GENERATION
        #
        # 1. Continent mask  — very low frequency gradient fBm.
        #    Controls where land vs. ocean lies; the same patch of
        #    coordinates always yields the same continental shape.
        #
        # 2. Mountain ridges — ridged multifractal, multiplied by
        #    the continent so mountains only grow on land.
        #
        # 3. Detail          — fine-scale gradient fBm for local texture.
        # ----------------------------------------------------------
        continent_scale = self.scale * continent_freq_factor

        # Build a small separate grid at the same world coordinates but
        # at continent scale (reuse X, Y — fbm samples them with its own freq).
        continent = fbm(
            X, Y,
            max(3, self.octaves - 2),   # fewer octaves — continental shape is smooth
            self.persistence * 1.1,      # slightly higher persistence → broader humps
            self.lacunarity,
            continent_scale,
            self.seed + 9999
        )
        # Remap to [0, 1] — continent values become a mask weight.
        c_min, c_max = continent.min(), continent.max()
        continent = (continent - c_min) / (c_max - c_min + 1e-8)
        ...
        # Mask mountains by the continent (ridges only where land is high).
        # Using continent² sharpens the land/ocean boundary.
        mountains *= continent ** 2
        ...
        detail = fbm(
            ...
            self.persistence,
            self.lacunarity,
            self.scale * 2.5,   # higher base frequency for fine detail
            self.seed + 7777
        )
        ...
        # Legacy slope-based damping — disabled by default (alpha=0).
        # When non-zero it artificially flattens steep ridges; keep at 0
        # unless you specifically need to suppress high-frequency spikes.
        if slope_reduction_alpha > 0.0:
            terrain = self.slope_based_height_reduction(
                terrain, lim=lim, alpha=slope_reduction_alpha)
        ...
        if thermal_iters > 0:
            # talus is expressed as a coordinate-space slope (height per coord-unit).
            # Multiply by pixel_dx to get the pixel-space height-diff threshold that
            # thermal_erosion expects. This keeps the same physical angle at every LOD.
            talus_px = talus * pixel_dx
            terrain = thermal_erosion(
                terrain,
                thermal_iters,
        ...
        if particle_droplets > 0:
            # particle_max_path is in coordinate units → convert to pixels.
            max_path_px = max(1, int(particle_max_path / pixel_dx))
            # particle_min_slope: coordinate-space slope → pixel space
            min_slope_px = particle_min_slope * pixel_dx

            # Scale droplet count proportionally to pixel area so that erosion
            # density (droplets per pixel) stays constant across resolutions and
            # zoom levels.  `particle_droplets` is treated as the intended count
            # for a 512×512 reference grid covering the same coordinate area.
            ref_pixels   = 512 * 512
            area_ratio   = (width * height) / ref_pixels
            scaled_drops = max(1, int(particle_droplets * area_ratio))
        ...
        # ----------------------------------------------------------
        # DRAINAGE-FLOW RIVER CARVING
        #
        # Computes a D8 catchment-area (flow accumulation) map, then
        # lowers cells proportional to √(flow) — matching the empirical
        # stream-power law.  Run *after* erosion so that rivers follow
        # the eroded valley network rather than the raw fBm.
        # Set flow_threshold=0 to skip this step entirely.
        # ----------------------------------------------------------
        if flow_threshold > 0:
            norm_t = self.normalize(terrain)      # work in [0,1] space
            accum  = flow_accumulation(norm_t)
            norm_t = carve_rivers(
                norm_t, accum,
                threshold   = flow_threshold,
            ...
            # Re-blend: preserve original height range, replace only carved delta.
            mn, mx  = terrain.min(), terrain.max()
            terrain = norm_t * (mx - mn) + mn
    ...
    def plot_slope_angle_histogram(self, terrain, lim=(0, 1, 0, 1), world_size_m=100_000, max_altitude=1000):
        ...
        xmin, xmax, ymin, ymax = lim
        h, w = terrain.shape
        # physical metres per pixel in x and y
        cell_x = (xmax - xmin) * world_size_m / w
        cell_y = (ymax - ymin) * world_size_m / h

        # convert normalised height to metres, then compute gradient in metres/metre
        terrain_m = terrain * max_altitude
        grad_y, grad_x = np.gradient(terrain_m, cell_y, cell_x)
        slope = np.arctan(np.sqrt(grad_x ** 2 + grad_y ** 2)) * (180.0 / np.pi)
